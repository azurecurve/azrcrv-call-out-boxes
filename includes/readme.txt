...
### [Version 2.0.0](https://github.com/azurecurve/azrcrv-call-out-boxes/releases/tag/v2.0.0)
 * Rebuild plugin on the current azurecurve modular plugin pattern (namespaced includes/ files), matching Related Posts and other current-generation azurecurve plugins; no change to the call-out-box/cob shortcode tags, the call-out-box custom post type, or stored settings/template data.
 * Add a "Process shortcodes within content" setting; the underlying option existed previously but had no way to enable it from the Settings screen.
 * Fix: remove Network Admin hooks that pointed at undefined functions and could fatal-error on multisite installs.
 * Fix: correct text domain used on the plugin's "Settings" action link.
 * Fix: use proper attribute escaping for call-out box inline styles, instead of an incorrectly-applied translation function.
 * Fix: avoid an unnecessary template lookup when the shortcode is used without a key parameter.
 * Fix: the "Integrate with Icons" setting is now correctly respected everywhere an icon could be shown (front-end shortcode output and the admin example previews on the Settings page and template edit screen); previously an icon could appear whenever the Icons plugin was simply active, regardless of whether integration had been turned on.
 * Add an Instructions tab to the Settings screen, and split settings across Settings / Instructions / Other Plugins tabs for consistency with other azurecurve plugins.
 * Increase minimum required PHP version to 8.2 and add a Requires CP header.

### [Version 1.5.5](https://github.com/azurecurve/azrcrv-call-out-boxes/releases/tag/v1.5.5)
...
### [Version 1.5.2](https://github.com/azurecurve/azrcrv-call-out-boxes/releases/tag/v1.5.2)
 * Update readme files.
 ...
 * Broken Links - [details](https://development.azurecurve.co.uk/classicpress-plugins/broken-links/) / [download](https://github.com/azurecurve/azrcrv-breadcrumbs/releases/latest/)
 ...
 * To X - [details](https://development.azurecurve.co.uk/classicpress-plugins/to-x/) / [download](https://github.com/azurecurve/azrcrv-to-x/releases/latest/)
