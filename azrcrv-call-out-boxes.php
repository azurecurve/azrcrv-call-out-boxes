...
function azrcrv_cob_update_options($option_name, $new_options, $is_network_site){
	if ($is_network_site == true){
		if (get_site_option($option_name) === false){
			...
			update_site_option($option_name, azrcrv_cob_update_default_options($new_options, get_site_option($option_name)));
		}
	}else{
		if (get_option($option_name) === false){
			add_option($option_name, $new_options);
		}else{
			update_option($option_name, azrcrv_cob_update_default_options($new_options, get_option($option_name)));
		}
	}
}
...
function azrcrv_cob_update_default_options( &$default_options, $current_options ) {
    ...
    foreach ($default_options as $key => &$value) {
        if (is_array( $value) && isset( $updated_options[$key ])){
            $updated_options[$key] = azrcrv_cob_update_default_options($value, $updated_options[$key], true);
        } else {
